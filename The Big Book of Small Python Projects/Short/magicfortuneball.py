...
def slowSpacePrint(text, interval=0.1):
    '''Slowly display text with spaces in between each letter and lower case i's.'''
...
while True:
    ...
    if '?' in response:
        print()
        print('Replies have question mark! Pode continuar!'.upper())
        print()
...
slowSpacePrint(random.choice(answer), 0.05)

resposta = [
    'VAMOS FICANDO POR AQUI!',
    'ACHO QUE VOCÊ AINDA QUER BRINCAR MAIS UM POUCO',
    'IA GOSTAR DE BRINCAR COM A ELEONORA AGORA, VER AQUELE SORRISINHO DE CANTO',
    'TAMBÉM IA GOSTAR DE ESCUTAR AQUELE SUTAQUE FOFO DELA',
    'VAMOS QUE VAMOS, QUEM SABE!'
    ]
slowSpacePrint(random.choice(resposta), 0.06)